...
- **Single most important next thing:** Run Robinhood probe (`scripts/robinhood_probe.py`) after adding `ROBINHOOD_AUTH_TOKEN` to `.env` — reconcile all `# TODO-VERIFY` entries before any live order. Parallel: wire baselines into `run_baseline.py` and run 2-5y historical data for rules-baseline excess CAGR + deflated SR (DoD requirement).
